...
var rxFrameId = -1;
const MOUSE_EVENTS = ["click", "dblclick", "mouseup", "mousedown", "contextmenu", "mouseenter", "mouseleave", "mousemove", "mouseout", "mouseover"];
const FOCUS_EVENTS = ["focus", "blur", "focusin", "focusout"];
const UI_EVENTS = ["abort", "beforeunload", "error", "load", "resize", "scroll", "select", "unload", "change", "submit", "input", "reset"];
const WHEEL_EVENTS = ["wheel"];
const CLIPBOARD_EVENTS = ["copy", "cut", "paste"];
const KEYBOARD_EVENTS = ["keydown", "keyup"];
const DRAG_EVENTS = ["drag", "dragstart", "dragend", "dragenter", "dragleave", "dragover", "drop"];
...
function raiseEvent(eventName, usedTempId, id, r) {
    ...
    const eventNameLowered = eventName.toLowerCase();
    ...
    try {
        ...
        if (MOUSE_EVENTS.includes(eventNameLowered)) {
            const ev = new MouseEvent(eventNameLowered);
            _rxtag.dispatchEvent(ev);
        } else if (FOCUS_EVENTS.includes(eventNameLowered)) {
            const ev = new FocusEvent(eventNameLowered);
            _rxtag.dispatchEvent(ev);
        } else if (UI_EVENTS.includes(eventNameLowered)) {
            const ev = new UIEvent(eventNameLowered);
            _rxtag.dispatchEvent(ev);
        } else if (WHEEL_EVENTS.includes(eventNameLowered)) {
            const ev = new WheelEvent(eventNameLowered);
            _rxtag.dispatchEvent(ev);
        } else if (CLIPBOARD_EVENTS.includes(eventNameLowered)) {
            const ev = new ClipboardEvent(eventNameLowered);
            _rxtag.dispatchEvent(ev);
        } else if (KEYBOARD_EVENTS.includes(eventNameLowered)) {
            const ev = new KeyboardEvent(eventNameLowered);
            _rxtag.dispatchEvent(ev);
        } else if (DRAG_EVENTS.includes(eventNameLowered)) {
            const ev = new DragEvent(eventNameLowered);
            _rxtag.dispatchEvent(ev);
